...
const options_entry cli_option_entries[] =
{
    ...
    { CLICOMMAND_LISTMEDIAXML   ";lmx",     "0",       OPTION_COMMAND,    "list available media for the system in XML format" },
...
void cli_frontend::listmediaxml(const std::vector<std::string> &args)
{
    ...
    info_xml_creator creator(m_options);
	creator.output_media(stdout, drivlist);
}
...
const cli_frontend::info_command_struct *cli_frontend::find_command(const std::string &s)
{
	static const info_command_struct s_info_commands[] =
	{
        ...
        { CLICOMMAND_LISTMEDIAXML,      0,  1, true,    &cli_frontend::listmediaxml,     "[system name]" },
